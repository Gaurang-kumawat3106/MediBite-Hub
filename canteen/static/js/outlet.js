...
/* ─── OUTLET: Toggle availability feedback ────────── */
function initToggleFeedback() {
  $$('.switch input[type="checkbox"]').forEach(checkbox => {
    checkbox.addEventListener('change', function () {
      const card  = checkbox.closest('.product-card');
      const label = card && card.querySelector('.toggle-label');
      const badge = card && card.querySelector('.img-status-badge');
      const isOn  = checkbox.checked;

      if (label) {
        label.textContent = isOn ? 'Enabled' : 'Disabled';
        label.className   = 'toggle-label' + (isOn ? ' on' : '');
      }
      if (badge) {
        badge.textContent = isOn ? 'Live' : 'Inactive';
        badge.className   = 'img-status-badge ' + (isOn ? 'live' : 'inactive');
      }
      if (card) {
        card.classList.toggle('unavailable', !isOn);
      }
      showToast(
        isOn ? 'Product is now Live!' : 'Product set to Inactive',
        isOn ? 'fa-circle-check' : 'fa-eye-slash'
      );
    });
  });
}

/* ─── OUTLET: Stat counter animation ─────────────── */
...
/* ─── OUTLET: Update-status form UX ──────────────── */
function initStatusUpdate() {
  $$('.o-update-form').forEach(form => {
...
/* ─── OUTLET: Confirm delete category ────────────── */
function initDeleteConfirm() {
  $$('.delete-cat-btn').forEach(link => {
    link.addEventListener('click', e => {
      if (!confirm('Delete this category? All its products will also be removed.')) {
        e.preventDefault();
      }
    });
  });
...
/* ─── OUTLET: Row hover highlight for table ──────── */
function initTableRowHighlight() {
  $$('.o-table tbody tr').forEach((tr, i) => {
