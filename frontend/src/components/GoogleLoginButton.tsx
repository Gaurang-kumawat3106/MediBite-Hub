...
export default function GoogleLoginButton({
  ...
  const handleManualClick = () => {
    ...
    try {
      ...
      const iframeBtn = googleBtnContainerRef.current?.querySelector(
        "div[role=button]"
      ) as HTMLElement;
      if (iframeBtn) {
        iframeBtn.click();
      ...
      console.error("Manual Google click error:", err);
    }
  };
      ...
      try {
        ...
        if (googleBtnContainerRef.current) {
          ...
          window.google.accounts.id.renderButton(googleBtnContainerRef.current, {
            ...
            size: "large",
            width: "340",
            text: buttonText.toLowerCase().includes("sign")
              ? "signup_with"
              : "continue_with",
            shape: "rectangular",
            logo_alignment: "left",
        ...
        <>
          ...
          {!isGsiReady && (
            <button
              ...
              className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl border border-gray-300 bg-white text-[#2b1b10] font-semibold text-sm hover:bg-gray-50 active:bg-gray-100 transition-all duration-200 shadow-xs cursor-pointer"
            >
